...
export default function BindDocumentModal({
  ...
  const docsQuery = useQuery({
    queryKey: ['closeout-bind-documents', projectId],
    queryFn: () =>
      apiGet<DocumentItem[]>(
        `/v1/documents/?project_id=${encodeURIComponent(projectId)}&limit=200`,
      ),
  ...
  const filtered = useMemo(() => {
    ...
    // Soft-rank documents whose category / discipline matches the slot.
    ...
    const list = q
      ? docs.filter((d) => {
          const hay = `${d.name || ''} ${d.category || ''} ${d.discipline || ''}`.toLowerCase();
          return hay.includes(q);
        })
      : docs;
    return [...list].sort((a, b) => Number(matchesSlot(b)) - Number(matchesSlot(a)));
  }, [docs, search, slot.category, slot.discipline]);
