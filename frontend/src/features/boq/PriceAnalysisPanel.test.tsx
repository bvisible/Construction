...
import type { PriceAnalysisResponse } from './api';
...
beforeEach(() => {
  ...
  // the `efb` key exists only when the request carried `preset=efb`.
  mockApiGet.mockImplementation(async (path: string) =>
    path.includes('preset=efb') ? EFB_ANALYSIS : SPLIT_ANALYSIS,
  );
});
...
describe('PriceAnalysisPanel', () => {
  it('asks the position price-analysis endpoint for the international preset first', async () => {
    renderPanel();

    await waitFor(() => expect(mockApiGet).toHaveBeenCalled());
    expect(requestedPaths()[0]).toBe(
      '/v1/boq/positions/pos-1/price-analysis/?preset=international',
    );
  });
  ...
  it('says a rate has not been broken down instead of drawing a one-line table', async () => {
    mockApiGet.mockResolvedValue(UNSPLIT_ANALYSIS);
    renderPanel(false);
