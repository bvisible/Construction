...
Close the app before you start. On Windows, download the new installer and run it. It notices the version you already have and offers either to remove it first or to write the new files over it, and removing it first, which is the option already selected for you, is the one to take. Windows then shows you the old version's own uninstall window in the middle of the upgrade, which looks alarming but is only the old program files being cleared out. Your `.openestimate` folder is not part of that, so your projects, users and settings are all still there when the new version starts. The first start after an upgrade can take longer than usual while the database brings itself up to date, and that is normal.

If that uninstall window offers a checkbox called "Delete the application data", you can leave it alone. It does not remove your projects, and what it does remove is described in the next section.
