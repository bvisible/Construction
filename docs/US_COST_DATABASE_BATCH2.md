...
**Scope:** 13 fresh cost items for concrete curbing, asphalt, utility conduits, storm pipe, crawlspace prep, and foundation drains
**Prerequisite:** Pilot Batch 1 (12 items) complete, `data/us_tn_sitework_costs.json`
...
Grow the USA_TENNESSEE cost database with 13 new composite cost items so it covers the 4 remaining TCG BedRock jobs:
...
Together these jobs account for roughly $235K in TCG bids that the pilot batch did not reach.

---

## Scope: 13 New Cost Items

### Concrete and Paving (4 items)
...
### Utility Conduits and Trenches (3 items)
| Code | Description | Unit | Est. Rate |
|------|-------------|------|-----------|
...
| UT-GAS-01 | Gas line trench, 3' wide x 3' deep, stone dust backfill + yellow tracer tape (no pipe) | LF | $16.00 |
...
| Code | Description | Unit | Est. Rate |
|------|-------------|------|-----------|
| EXC-CRW-01 | Crawlspace and footer excavation, precision (plus or minus 1/4"), stockpile on site, 5 foundations | CY | $14.00 |
...
Pulled from the TCG estimate notes:
- 75 LF concrete curb and gutter along the street frontage
- 844 LF concrete ribbon curb around the driveway/parking perimeter
- Subgrade excavation, stone base, 3500 PSI concrete, joints
- Excavation/grading for the driveway ramp, sidewalk, walkways
- Road widening: 4' x 125' area (500 SF), 8" stone, 3.5" asphalt
- Repair utility trenches on the street with asphalt
- Mill and overlay the existing curb
- Credit: remove ribbon curb and walkway excavation ($37,000)

**BOQ positions for this job:**
...
Pulled from the TCG estimate notes:
- 1300 LF of total trenching (3' deep) for all utilities
- Sanitary sewer: 4" schedule 40 pipe from the street to each house
- Water service: 4" schedule 40 pipe from the street to each house
- Electrical conduit: 4" conduit in trench, 12" stone dust, red tracer tape
- Communications conduit: in the same trench as the electrical
- Gas line trench: 3' wide x 3' deep, stone dust backfill, yellow tracer tape (pipe by others)
- 125 LF of 18" RCP with 2 precast concrete headwalls and outlet protection
...
| 05.30.0010 | Gas line trench (3'x3', stone dust backfill + yellow tape, pipe by others) | 400 | LF | UT-GAS-01 | $16.00 |
...
Pulled from the TCG estimate notes:
- Excavate the crawlspace and footers for 5 foundation areas
- Install 4" corrugated perforated pipe around the perimeter of 5 foundations
- Surrounded by 12" #57 stone, wrapped in geotextile
- Pipe led to the storm drain
- Fine grade the crawlspace areas plus vapor barrier plus 4" #57 stone (5 foundations)
- Fine grade the garage areas (stone plus concrete by others) (5 areas)
- Fine grade the porch areas (stone plus concrete by others) (5 areas)
...
- Establish the finished foundation elevation
- Cut topsoil and stockpile
- Excavate and level the site for building pads
...
Every item uses the same resource-based methodology as Pilot Batch 1:
...
- **Material rates:** Nashville market estimates (extend `data/material_rates.json` with the new codes below)
...
Each item below carries its full component array. Validate with:
```python
...
| Excavator 20T (trench 18" wide x 3' deep) | equipment | 0.06 hr | $60.94/hr | $3.66 |
...
| Excavator 20T (3' wide x 3' deep trench) | equipment | 0.10 hr | $60.94/hr | $6.09 |
...
|-----------|------|--------|-----------|------|
| Construction Laborer, 2 workers x 8 hr (SOC 47-2061) | labor | 16 hr | $22.45/hr | $359.20 |
...
See the Pilot Batch 1 document for the full detail on:
...
All 13 plus 1 (SIT-LAY-01) items go into:

**`data/us_tn_concrete_utilities_costs.json`**

Same schema as Pilot Batch 1. Each item carries:
```json
{
  "code": "CON-CUR-01",
  "description": "...",
...
- [ ] 1. Extend `data/material_rates.json` with the 13 new material codes
- [ ] 2. Look up TDOT 2024 bid prices for the 8 new comparison items into `data/tdot_bid_prices.json`
- [ ] 3. Build 14 CostItems with full component arrays into `data/us_tn_concrete_utilities_costs.json`
- [ ] 4. Validate `rate == sum(components.cost)` for each item (tolerance plus or minus $0.01)
- [ ] 5. Validate that there are no negative rates and no zero components
- [ ] 6. Import Batch 2: `POST /api/v1/costs/bulk/` with the new JSON
- [ ] 7. Verify in OCERP: search for the `USA_TENNESSEE` region (it should now show 26 items)
- [ ] 8. Create BOQ positions for all 4 remaining jobs (see the BOQ tables above)
- [ ] 9. Compare OCERP totals against the BedRock CSV (see comparison tables below)
- [ ] 10. Compare OCERP component-level totals against TDOT bid prices
- [ ] 11. Document the validation report (extend `docs/validation_report.md`)
- [ ] 12. Update `data/IMPORT_INSTRUCTIONS.md` with the Batch 2 items
...
|-----|--------------|---------------------|-------|
| Concrete Curb (38447) | $61,154 | $45K to $55K | After the $37K credit, the effective net is around $18K |
| Trenching (38522) | $51,842 | $45K to $55K | Most items have a strong cost item match |
| Excavation (38577) | $36,299 | $30K to $40K | Crawlspace plus perimeter drain make up the bulk of the cost |
| Additional Work (38578) | $85,979 | $70K to $90K | Excavator rental is $13.5K; the rest is bulk earthwork |

Aim for plus or minus 25% variance on the first pass; refine the rates if you are more than 30% off.
...
| Risk | Mitigation |
|------|-----------|
| The asphalt paver hourly rate is estimated (not in USACE data) | Mark the source as `estimated`; refine with rental house pricing |
| Monthly equipment rental rates are rough estimates | Call local equipment rental companies for current Nashville rates |
| Some items (SIT-LAY-01) are lump sum, hard to validate per-unit | Accept a reasonable range and note the methodology |
| Credit items in the TCG estimate reduce totals | Apply credits as separate negative positions with notes |
| UT-COM-01 shares a trench with UT-ELC-01, risk of double-counting | UT-COM-01 deliberately excludes trench labor and equipment |
| Some items may need manual entry (lump sum credits, mill/overlay) | Create these as `source: "manual"` positions in the BOQ, not via the cost DB |

---

## Import Script Update

Update `scripts/import_cost_database.py` so it takes a `--file` argument and can import any cost JSON:
...
Once Batch 2 is validated, the database will cover **all 7 TCG jobs**. Future development could add:

- **Batch 3** (around 10 items): Erosion controls (silt fence, straw mulch, construction fence, erosion blankets)
- **Batch 4** (around 8 items): Additional utility items (fire hydrants, water meters, backflow preventers, valve boxes)
- **Batch 5** (around 6 items): Site finishing (seeding, sod, landscaping prep, topsoil placement)
