...
| 07 | `07-olares-embeddings.patch` | `app/core/olares_embeddings.py` (nouveau, **NEOFFICE FILE**) + `app/core/vector.py` (`get_embedder` / `_resolve_active_model`) + `app/modules/costs/qdrant_adapter.py` (`_get_encoder`) | Déporte **tous** les embeddings vers Olares → zéro modèle ML local sur les instances 4 Go. Dense général → `embeddings.noraai.ch/v1` (`qwen3-embedding:8b`, 4096-d) ; CWICR hybride dense+sparse → `bge.noraai.ch/embed` (`bge-m3`, 1024-d). Httpx only (pas de SDK — cohérent avec la politique `ai_client`). Gated par site_config : `oce_olares_api_key` + `oce_olares_embeddings` (dense, défaut ON si clé présente) + `oce_olares_bge_url` (CWICR, OFF tant qu'absent) ; fallback modèle local si Olares non configuré (dev). Réutilise le provider Olares du patch 02. Validé osiris 2026-06-13 (encode 4096-d + match hybride, `torch=False`). | Permanent (archi Olares). Réviser si upstream expose un backend d'embedding remote natif. |
