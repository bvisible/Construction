...
"""GAEB importer regression against the official BVBS 3.3 Pruefdatei (X83).

The certification file deliberately stresses the corners a real-world LV can
hit, and each of them maps to a bug class the audit found in a client-side
import path:
...
Run::
    ...
    cd backend
    python -m pytest tests/unit/test_gaeb_pruefdatei_import.py -v --tb=short
"""
...
_FIXTURE = Path(__file__).resolve().parents[1] / "fixtures" / "gaeb" / "pruefdatei_3.3_x83.x83"

# First bytes of the base64-encoded JPEG embedded in the fixture's Langtext.
_JPEG_B64_MARKER = "/9j/4AAQ"

# The four Indexpositionen of the certification file: same RNoPart as their
# base position, distinguished ONLY by RNoIndex.
_INDEXED_OZS = (
    "001.001.0010.1",
    "001.001.0010.A",
    "999.999.9999.y",
...
class TestPruefdateiAllPositions:
    def test_all_27_positions_imported(self, pruefdatei_result) -> None:
        """The official certification file yields ALL 27 items, no errors."""
        items = _items(pruefdatei_result)
        assert len(items) == 27, f"expected 27 items, got {len(items)}"
        assert pruefdatei_result.errors == []
        assert pruefdatei_result.skipped == 0

    def test_rno_index_is_part_of_the_oz(self, pruefdatei_result) -> None:
        """Indexpositionen keep their RNoIndex suffix: 27 DISTINCT ordinals."""
        ordinals = [p.ordinal for p in _items(pruefdatei_result)]
    ...
    def test_rno_index_recorded_in_metadata(self, pruefdatei_result) -> None:
        """Each Indexposition carries its raw index for round-trip export."""
        by_ordinal = {p.ordinal: p for p in _items(pruefdatei_result)}
        for oz in _INDEXED_OZS:
            index = oz.rsplit(".", 1)[-1]
            assert by_ordinal[oz].metadata.get("gaeb_rno_index") == index


class TestPruefdateiPhaseDetection:
    def test_x83_detected_as_x83(self, pruefdatei_result) -> None:
        """Phase comes from Award/DP (83), never from the absence of prices.

        The Pruefdatei is unpriced, so a "has prices => X83, else X81"
        heuristic would misreport it. The importer must say x83.
        """
        assert pruefdatei_result.metadata["da_kind"] == "x83"
        assert _items(pruefdatei_result)[0].metadata["gaeb_da_kind"] == "x83"


class TestPruefdateiEmbeddedGraphics:
    def test_base64_blob_never_reaches_description(self, pruefdatei_result) -> None:
        """The inline JPEG is stripped from every description."""
        for pos in pruefdatei_result.positions:
            assert _JPEG_B64_MARKER not in (pos.description or ""), (
                f"base64 graphic leaked into description of {pos.ordinal}"
            )

    def test_base64_blob_never_reaches_metadata(self, pruefdatei_result) -> None:
    ...
    def test_graphic_item_keeps_its_text(self, pruefdatei_result) -> None:
        """Stripping the graphic must not throw away the human text with it."""
        by_ordinal = {p.ordinal: p for p in _items(pruefdatei_result)}
        # 001.002.0010 is the item whose Langtext embeds the JPEG.
        item = by_ordinal["001.002.0010"]
        assert item.description.startswith("Boden f")
        assert len(item.description) > 40

    def test_descriptions_fit_the_position_schema(self, pruefdatei_result) -> None:
