...
def ordered_input_files(
    ...
    retry_chunk_pairs = {
        pair_for_input(path)
        for path in files
        if "retry" in path.name.lower() and "__part" in path.stem
    }
    if retry_chunk_pairs:
        files = [
            path
            for path in files
            if pair_for_input(path) not in retry_chunk_pairs or "__part" in path.stem
        ]
    retry_pairs = {
        pair_for_input(path) for path in files if "retry" in path.name.lower()
    }
    if retry_pairs:
        files = [
            path
            for path in files
            if pair_for_input(path) not in retry_pairs or "retry" in path.name.lower()
        ]
...
def status() -> dict:
    ...
    return {
        ...
        "next_input_files": [str(path) for path in files if str(path) not in known][
            :10
        ],
    }
...
def parse_outputs() -> dict:
    ...
    for pair_dir in sorted(OUTPUTS_DIR.glob("llm_batches_*_*")):
        shard_files = sorted(
            (pair_dir / "openrouter_shards").glob("*_openrouter.parquet")
        )
        if not shard_files:
            continue
        ...
        translations = pd.concat(
            [pd.read_parquet(path) for path in shard_files], ignore_index=True
        )
        ...
        final_qa_path.write_text(
            json.dumps(final_qa, ensure_ascii=False, indent=2), encoding="utf-8"
        )
